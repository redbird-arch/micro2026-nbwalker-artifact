...
WORKTREE_PATH = os.path.join(WORKTREE_ROOT, "AE-NBWalker")
MGPUSIM_PATH = f"{WORKTREE_ROOT}/AE-NBWalker/simulator/mgpusim/"
...
def main():
    ...
    if args.server:
        generate_runners(name="baseline", yaml_path=f"{PROJECT_ROOT}/configs/baselineMMU.yaml", use_amr=False)
        generate_runners(name="nbwalker", yaml_path=f"{PROJECT_ROOT}/configs/nbwalker.yaml", use_amr=False)
        generate_runners(name="nbwalker+amr", yaml_path=f"{PROJECT_ROOT}/configs/nbwalker.yaml", use_amr=True)
        generate_runners(name="infinite", yaml_path=f"{PROJECT_ROOT}/configs/infiniteMMU.yaml", use_amr=False)
    else:
        generate_runners_on_desktop(name="baseline", yaml_path=f"{PROJECT_ROOT}/configs/baselineMMU.yaml", use_amr=False)
        generate_runners_on_desktop(name="nbwalker", yaml_path=f"{PROJECT_ROOT}/configs/nbwalker.yaml", use_amr=False)
        generate_runners_on_desktop(name="nbwalker+amr", yaml_path=f"{PROJECT_ROOT}/configs/nbwalker.yaml", use_amr=True)
        generate_runners_on_desktop(name="infinite", yaml_path=f"{PROJECT_ROOT}/configs/infiniteMMU.yaml", use_amr=False)

...
WORKTREE_PATH = os.path.join(WORKTREE_ROOT, "AE-SoftWalker")
MGPUSIM_PATH = f"{WORKTREE_ROOT}/AE-SoftWalker/simulator/mgpusim/"
...
def generate_runners(name, yaml_path=None):
...
def generate_runners_on_desktop(name, yaml_path=None):
    base_output_dir = os.path.join(PROJECT_ROOT, "runs", name)
    os.makedirs(base_output_dir, exist_ok=True)
...
def main():
    ...
    if args.server:
        generate_runners(name="softwalker", yaml_path=f"{PROJECT_ROOT}/configs/baselineMMU.yaml")
    else:
        generate_runners_on_desktop(name="softwalker", yaml_path=f"{PROJECT_ROOT}/configs/baselineMMU.yaml")

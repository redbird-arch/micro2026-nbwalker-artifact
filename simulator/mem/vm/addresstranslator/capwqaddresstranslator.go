...
// CaPWQAddressTranslator is a component that forwards the read/write requests with
// the address translated from virtual to physical.
type CaPWQAddressTranslator struct {
...
func (t *CaPWQAddressTranslator) SetTranslationProvider(p interface{}) {
	t.translationProvider = p.(akita.Port)
}

// SetLowModuleFinder sets the table recording where to find an address.
func (t *CaPWQAddressTranslator) SetLowModuleFinder(lmf cache.LowModuleFinder) {
	t.lowModuleFinder = lmf
}

// GetCtrlPort sets the table recording where to find an address.
func (t *CaPWQAddressTranslator) GetCtrlPort() (port akita.Port) {
	port = t.CtrlPort
	return
}

// GetTopPort sets the table recording where to find an address.
func (t *CaPWQAddressTranslator) GetTopPort() (port akita.Port) {
	port = t.TopPort
	return
}

// GetTranslationPort sets the table recording where to find an address.
func (t *CaPWQAddressTranslator) GetTranslationPort() (port akita.Port) {
	port = t.TranslationPort
	return
}

// GetBottomPort sets the table recording where to find an address.
func (t *CaPWQAddressTranslator) GetBottomPort() (port akita.Port) {
	port = t.BottomPort
	return
}

// Tick updates state at each cycle.
func (t *CaPWQAddressTranslator) Tick(now akita.VTimeInSec) bool {
	madeProgress := false
...
func (t *CaPWQAddressTranslator) runPipeline(now akita.VTimeInSec) bool {
	madeProgress := false
...
// func getKind(t *CaPWQAddressTranslator) (s string) {
// 	// fmt.Println(t.Name())
...
func (t *CaPWQAddressTranslator) translate(now akita.VTimeInSec) bool {
	...
	if req.Meta().PTW {
		return t.parseFromWalker(now, req)
	}
...
func (t *CaPWQAddressTranslator) parseFromWalker(
...
func (t *CaPWQAddressTranslator) parseTranslation(now akita.VTimeInSec) bool {
...
func (t *CaPWQAddressTranslator) sendToBottom(now akita.VTimeInSec) bool {
	for _, transaction := range t.transactions {
		if transaction.translationDone {
			reqFromTop := transaction.incomingReqs[0]

			var translatedReq mem.AccessReq

			if reqFromTop.Meta().PTW {
				translatedReq = t.createPageWalkerReadReq(
					reqFromTop,
				)
			} else {
...
func (t *CaPWQAddressTranslator) respond(now akita.VTimeInSec) bool {
...
func (t *CaPWQAddressTranslator) createTranslatedReq(
	req mem.AccessReq,
	page device.Page,
) mem.AccessReq {
	switch req := req.(type) {
...
func (t *CaPWQAddressTranslator) createTranslatedReadReq(
	req *mem.ReadReq,
	page device.Page,
) *mem.ReadReq {
...
func (t *CaPWQAddressTranslator) createPageWalkerReadReq(
	reqFromTop mem.AccessReq,
) *mem.ReadReq {
	req := reqFromTop.(*mem.ReadReq)

	clone := mem.ReadReqBuilder{}.
		WithSrc(t.BottomPort).
		WithDst(t.lowModuleFinder.Find(req.Address)).
...
func (t *CaPWQAddressTranslator) createTranslatedWriteReq(
	req *mem.WriteReq,
	page device.Page,
) *mem.WriteReq {
	offset := req.Address % (1 << t.log2PageSize)
...
func (t *CaPWQAddressTranslator) addrToPageID(addr uint64) uint64 {
	return (addr >> t.log2PageSize) << t.log2PageSize
}

func (t *CaPWQAddressTranslator) findTranslationByReqID(id string) *transaction {
...
func (t *CaPWQAddressTranslator) removeExistingTranslation(trans *transaction) {
...
func (t *CaPWQAddressTranslator) isReqInBottomByID(id string) bool {
...
func (t *CaPWQAddressTranslator) findReqToBottomByID(id string) reqToBottom {
	for _, r := range t.inflightReqToBottom {
		if r.reqToBottom.Meta().ID == id {
			return r
		}
...
func (t *CaPWQAddressTranslator) removeReqToBottomByID(id string) {
...
func (t *CaPWQAddressTranslator) handleCtrlRequest(now akita.VTimeInSec) bool {
	req := t.CtrlPort.Peek()
	if req == nil {
		return false
	}
...
func (t *CaPWQAddressTranslator) handleFlushReq(
	now akita.VTimeInSec,
	req *mem.ControlMsg,
) bool {
...
func (t *CaPWQAddressTranslator) handleRestartReq(
	now akita.VTimeInSec,
	req *mem.ControlMsg,
) bool {
	rsp := mem.ControlMsgBuilder{}.

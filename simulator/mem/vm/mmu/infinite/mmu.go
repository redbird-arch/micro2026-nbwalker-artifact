...
func (impl *MMUImpl) Tick(now akita.VTimeInSec) bool {
	...
	if len(impl.pageWalkers) > 0 {
		tracing.StartTask(
			...
			impl,
			"num_active_walkers",
			strconv.Itoa(int(len(impl.pageWalkers))),
			nil,
		)
		tracing.StartTask(
			"",
...
func (impl *MMUImpl) walkPageTable(now akita.VTimeInSec) bool {
	...
	for _, walker := range impl.pageWalkers {
		...
		inflightTrans := walker.inflightTrans
		...
		if walker.inflightTrans != nil {
			tmp = append(tmp, walker)
		}
	}

	impl.pageWalkers = tmp
...
func (impl *MMUImpl) parseFromTop(now akita.VTimeInSec) bool {
	...
	walker := &PageWalkerImpl{
		inflightTrans: &translationInPipeline,
	}

	impl.pageWalkers = append(impl.pageWalkers, walker)

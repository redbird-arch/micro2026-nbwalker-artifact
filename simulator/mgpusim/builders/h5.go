...
func MakeH5Builder() H5Builder {
	// TODO: should this be using new? is the object being allocated on the stack?
	cbp := CommonBuilder{}
	b := H5Builder{CommonBuilder: &cbp}
	b.SetDefaultCommonBuilderParams()
	return b
}

func (b H5Builder) Build(name string, id uint64) *mgpusim.GPU {
	b.createGPU(name, id)
...
func (b *H5Builder) connectL1TLBToL2TLBMagic() {
...
func (b *H5Builder) setupInterchipNetwork() {
...
func (b *H5Builder) InterChipletPorts(c *Chiplet) []akita.Port {
	ports := []akita.Port{
...
func (b *H5Builder) buildL2TLB(chiplet *Chiplet) {

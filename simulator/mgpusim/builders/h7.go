...
// MakeH7GPUBuilder provides a GPU builder that can builds MCM GPU.
func MakeH7GPUBuilder() H7GPUBuilder {
	// TODO: should this be using new? is the object being allocated on the stack?
	cbp := CommonBuilder{}
	b := H7GPUBuilder{CommonBuilder: &cbp}
	b.SetDefaultCommonBuilderParams()
	return b
}

func (b H7GPUBuilder) Build(name string, id uint64) *mgpusim.GPU {
	b.createGPU(name, id)
...
func (b *H7GPUBuilder) createRemoteAddrTransTable() *cache.XORLowModuleFinder {
...
func (b *H7GPUBuilder) configRemoteAddressTranslationUnit(chiplet *Chiplet,
...
func (b *H7GPUBuilder) connectL1TLBToL2TLB(chiplet *Chiplet) {
...
func (b *H7GPUBuilder) buildL2TLB(chiplet *Chiplet) {

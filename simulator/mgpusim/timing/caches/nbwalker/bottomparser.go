...
func (p *bottomParser) processDataReady(
	...
	if trans.fromWalker {
		return p.processPTEsDataReady(now, dr, trans)
	}
...
func (p *bottomParser) processPTEsDataReady(
	now akita.VTimeInSec,
	dr *mem.DataReadyRsp,
	trans *transaction,
) bool {
	...
	cachelineID := (addr >> p.cache.log2BlockSize) << p.cache.log2BlockSize
	cachelineEnd := cachelineID + uint64(1<<p.cache.log2BlockSize)

	for _, req := range mshrEntry.Requests {
		t := req.(*transaction)

		if t.write.Address < cachelineID ||
			t.write.Address >= cachelineEnd {
			continue
		}

		offset := t.write.Address - cachelineID
		t.data = data[offset : offset+8]
		t.done = true
